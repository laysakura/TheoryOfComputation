...
\textsf{emathG.sty}が\texttt{tpic-specials}で描画するのに対して,
\textsf{emathGps.sty}は\texttt{PostScript}で描画する点が異なります。
...
\begin{jquote}
\begin{verbatim}
\usepackage[debug]{emathGps}
\end{verbatim}
\end{jquote}
として，\verb+[debug]+オプション付でタイプセットし,次いで第2段階で
\verb+[debug]+オプションを取り去って
\begin{jquote}
\begin{verbatim}
\usepackage{emathGps}
\end{verbatim}
...
\section{折れ線グラフ}
現時点では，折れ線グラフは\textsf{pszahyou(*)}環境で記述することになります。
...
\begin{pszahyou*}[ul=10mm,yscale=.2](0,4)(0,35)
%--- 折れ線描画
  \OreG{\Drawline}{$\bullet$}{10,20,10,30}
  \OreG{\Hasen}{$\scriptscriptstyle\blacksquare$}{5,2,12,10}
  \OreG{\Drawline<dash={.3,.1,.05,.1}>}{$\diamondsuit$}{20,15,30,15}
  \OreG{\Drawline<iro=red>}{{\color{green}{$\ast$}}}{28,25,20,14}
%--- 目盛り線など
...
\begin{pszahyou*}[ul=10mm,yscale=.2,Hidariyohaku=2em,Sitayohaku=3em](0,4)(0,35)
  \OreG{\Drawline}{$\bullet$}{10,20,10,30}
  \OreG{\Hasen}{$\scriptscriptstyle\blacksquare$}{5,2,12,10}
  \OreG{\Drawline<dash={.3,.1,.05,.1}>}{$\diamondsuit$}{20,15,30,15}
  \OreG{\Drawline<iro=red>}{{\color{green}{$\ast$}}}{28,25,20,14}
...
\setborderwidth{2pt}\relax
\section{棒グラフ（縦）}
\subsection{\textsf{bouG}環境}
\textsf{emathGps.sty v 0.06α}で，
棒グラフを描画する環境\textsf{bouG}を新設しました。

これは，実質\textsf{pszahyou*}環境です。
簡単な一例をご覧ください。
...
上で述べたように，\textsf{bouG}環境は実質\textsf{pszahyou*}環境ですから，
引数の記述は\textsf{pszahyou*}環境に従います。
...
\textsf{bouG}環境特有のオプションは，\textsf{pszahyou*}環境の\verb+[...]+オプションに先立ち，
\verb+<...>+で指定します。

\begin{showEx}(.64,.3){\texttt{<boukankaku=..>}オプション}
\begin{bouG}<boukankaku=.2>[ul=8mm,%
    yscale=.5,Sitayohaku=1zh]%
    (0,4)(0,10)
  \bou{4}{A}
  \bou{9}{B}
  \bou{8}{C}
...
\begin{bouG}<bouxo=.5,boukankaku=.5>[ul=8mm,%
    yscale=.5,Sitayohaku=1zh]%
    (0,4)(0,10)
  \bou{4}{A}
  \bou{9}{B}
...
\subsubsection{背景色}
背景色を指定するには，\textsf{bouG}環境，すなわち\textsf{pszahyou}環境で
...
\begin{bouG}<boukankaku=.2>
  ...
  \Nuritubusi[nuriiro=silver]{%
    \trueLT\trueLB\trueRB\trueRT}
  \bou{4}{A}
  \bou{9}{B}
  \bou{8}{C}
\end{bouG}
...
\subsection{縦目盛り}
縦軸に目盛りを入れるには，\textsf{pszahyou*}環境の機能を利用します。
...
これは，実質\textsf{pszahyou*}環境です。縦横の単位長が異なる場合が多いでしょうから,
その場合は \textsf{emathPxy}を別途ロードしなければなりません。

簡単な一例をご覧ください。

\begin{showEx}(.54,.4){\textsf{yokobouG}環境}
\begin{yokobouG}%
...
上で述べたように，\textsf{yokobouG}環境は実質\textsf{pszahyou*}環境ですから，
引数の記述は\textsf{pszahyou*}環境に従います。
...
\textsf{yokobouG}環境特有のオプションは，\textsf{pszahyou*}環境の\verb+[...]+オプションに先立ち，
\verb+<...>+で指定します。
...
\begin{showEx}(.54,.4){\texttt{<bouyo=..>}オプション}
...
\subsubsection{背景色}
背景色を指定するには，\textsf{yokobouG}環境，すなわち\textsf{pszahyou}環境で
座標平面全体を背景色で塗りつぶし，その上に棒グラフなどを配置していきます。

\begin{showEx}(.54,.4){背景色}
\begin{yokobouG}<boukankaku=.2>
      [ul=8mm,xscale=.5,Hidariyohaku=1zh]%
      (0,10)(0,4)
  \def\NuriF{[nuriiro=cyan]}
...
\begin{showEx}(.54,.4){\texttt{<bouDosuuiti=..>}}
\begin{yokobouG}
    <bouDosuuiti=.5>%
    [ul=8mm,xscale=.5,Hidariyohaku=1zh]%
    (0,10)(0,4)
...
\begin{showEx}(.54,.4){\texttt{<bouDosuuopt=..>}}
\begin{yokobouG}
    <bouDosuuopt={(0,0)[c]},bouDosuuiti=.5>%
    [ul=8mm,xscale=.5,Hidariyohaku=1zh]%
    (0,10)(0,4)
...
\begin{showEx}(.54,.4){\texttt{<prDosuu=..>}}
\begin{yokobouG}
    <prDosuu={\colorbox{white}},%
    bouDosuuopt={(0,0)[c]},bouDosuuiti=.5>%
    [ul=8mm,xscale=.5,Hidariyohaku=1zh]%
...
\begin{showEx}(.54,.4){\texttt{<fboxsep=..>}}
\begin{yokobouG}
    <fboxsep=1pt,%
    prDosuu={\colorbox{white}},%
    bouDosuuopt={(0,0)[c]},bouDosuuiti=.5>%
...
\subsection{横目盛り}
横軸に目盛りを入れるには，\textsf{pszahyou*}環境の機能を利用します。
...
\begin{TbouG}<bouxo=.5,boukankaku=.5,%
    ...
    (0,3.5)(0,25)
  \Nuritubusi[nuriiro=gold]%
    {\trueLB\trueLT\trueRT\trueRB}
  \def\NuriFi{}
  \def\NuriFii{*<.1>}
